...
def _company_name(db: Session) -> str:
    row = db.get(Setup, "company_name")
    return row.value if row and row.value else "GLPACK"
...
def trial_balance(
    ...
    data = svc.get_trial_balance(db, period_start, period_end)
    if format == "pdf":
        pdf = pdf_svc.build_single_pdf("trial-balance", data, _company_name(db))
...
def profit_loss(
    ...
    _validate_period(period_start, period_end)
    data = svc.get_profit_loss(db, period_start, period_end)
    if format == "pdf":
        pdf = pdf_svc.build_single_pdf("profit-loss", data, _company_name(db))
...
def balance_sheet(
    ...
    _validate_period(period_start, period_end)
    data = svc.get_balance_sheet(db, period_start, period_end)
    if format == "pdf":
        pdf = pdf_svc.build_single_pdf("balance-sheet", data, _company_name(db))
...
def ledger_account(
    ...
    _validate_period(period_start, period_end)
    data = svc.get_ledger_account(db, code, period_start, period_end)
    if data is None:
        raise HTTPException(status_code=404, detail=f"Account '{code.upper()}' not found")
    if format == "pdf":
        pdf = pdf_svc.lines_to_pdf(
            [pdf_svc.render_ledger_account(data, _company_name(db))]
        )
...
def expense_schedule(
    ...
    _validate_period(period_start, period_end)
    data = svc.get_expense_schedule(db, period_start, period_end)
    if format == "pdf":
        pdf = pdf_svc.build_single_pdf("expense-schedule", data, _company_name(db))
...
def debtors_listing(
    ...
    _validate_period(period_start, period_end)
    data = svc.get_debtors_listing(db, period_start, period_end)
    if format == "pdf":
        pdf = pdf_svc.build_single_pdf("debtors-listing", data, _company_name(db))
...
def creditors_listing(
    ...
    _validate_period(period_start, period_end)
    data = svc.get_creditors_listing(db, period_start, period_end)
    if format == "pdf":
        pdf = pdf_svc.build_single_pdf("creditors-listing", data, _company_name(db))
...
def fixed_assets(
    ...
    _validate_period(period_start, period_end)
    data = svc.get_fixed_assets(db, period_start, period_end)
    if format == "pdf":
        pdf = pdf_svc.build_single_pdf("fixed-assets", data, _company_name(db))
...
def full_financial_statements(
    ...
    expense = svc.get_expense_schedule(db, period_start, period_end)
    pl = svc.get_profit_loss(db, period_start, period_end)
    bs = svc.get_balance_sheet(db, period_start, period_end)
    debtors = svc.get_debtors_listing(db, period_start, period_end)
    creditors = svc.get_creditors_listing(db, period_start, period_end)
    fa = svc.get_fixed_assets(db, period_start, period_end)
